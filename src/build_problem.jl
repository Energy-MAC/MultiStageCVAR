function apply_cc_constraints!(problem)
    ...
    constraint = PSI.add_cons_container!(optimization_container, :CC_constraint, collect(keys(restrictions)), time_steps)
    jump_model = PSI.get_jump_model(optimization_container)
    for t in time_steps, (k, v) in restrictions
        constraint[k, t] = JuMP.@constraint(jump_model, sum(commitment_variables[i, t] for i in v) <= 1)
    end
    return
end
...
function set_initial_commitment!(problem::PSI.OperationsProblem{MultiStageCVAR})
    ...
    info_map = Dict((PSY.get_name(d) => PSY.get_status(d)) for d in PSY.get_components( ThermalMultiStart, system))
...
function set_inputs_dic!(problem::PSI.OperationsProblem{MultiStageCVAR})
    ...
    N = size(problem.ext["area_solar_forecast_prob"], 2)
    p = fill(1 / N, N)
...
function get_sddp_model(problem::PSI.OperationsProblem{MultiStageCVAR})
    ...
    model = SDDP.MarkovianPolicyGraph(
        ...
        SDDP.@variables(sp, begin
            # Thermal Generation Cost
            cg[thermal_gens_names] >= 0
            # Balacing Reserve variables
            0 <= rsv_up[g ∈ balancing_devices_names_up] <= reserve_vars_up[g]*commit_vars[g]
            0 <= rsv_dn[g ∈ balancing_devices_names_dn] <= reserve_vars_dn[g]*commit_vars[g]
            # ACE
            ACE⁺ >= 0
            ACE⁻ >= 0
